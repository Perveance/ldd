...
static int __init my_init(void)
{
    ...
    printk("Module 2: INIT before trylock count = %d\n",
            atomic_read(&my_mutex.count));

    if (mutex_trylock(&my_mutex) == 0) {

        printk("Module 2: Failed to obtain mutex\n");
        return -EBUSY;

    } else {

        printk("Module 2: Mutex obtained. count = %d\n",
            atomic_read(&my_mutex.count));
...
static void __exit my_exit(void)
{
    ...
    printk("Module 2: EXIT. Before unlock count = %d\n",
            atomic_read(&my_mutex.count));

    mutex_unlock(&my_mutex);

    printk("Module 2: EXIT. After unlock count = %d\n",
            atomic_read(&my_mutex.count));
